...
def exact_op(tree, kid_ivs, kid_errs, out_iv):
    ...
    if op == '+':
        # x + x — это удвоение: экспонента сдвигается на единицу, мантисса не
        # меняется, округления нет. Мы за него платили, и дорого: в цепочке
        # sqrt(exp(a+a) * exp(b+b)) несуществующая ошибка удвоения проходила через
        # экспоненту, множилась на величину порядка пятидесяти и раздувала границу
        # вчетверо против реальной ошибки (найдено 26.09.2026).
        if tree[1] == tree[2]:
            return True
        for pos, kid in enumerate(tree[1:]):
...
def _square(tree):
    """Символический квадрат: заносим возведение внутрь, где оно бесплатно.

    Смысл приёма в том, что квадрат и корень гасят друг друга по относительной
    ошибке. Но если возводить в квадрат снаружи — (f·f) — добавляется лишнее
    округление, и выигрыш съедается: на exp(a)·exp(b) внешний квадрат дал
    2.379e-14 против исходных 1.668e-14, то есть хуже.

    Внутри же квадрат часто ТОЧЕН: exp(u)² = exp(u+u), а удвоение аргумента
    округления не даёт вовсе. Поэтому спускаем возведение по дереву до тех мест,
    где оно даром, и только у листьев ставим честное умножение.
    """
    op = tree[0]
    if op == 'exp':
        return ('exp', ('+', tree[1], tree[1]))
    if op == 'sqrt':
        return tree[1]
    if op in ('*', '/'):
        return (op, _square(tree[1]), _square(tree[2]))
    if op == 'neg':
        return _square(tree[1])
    return ('*', tree, tree)


def sqrt_square_candidates(front, domain, boxes=None):
    """Кандидат «через квадрат»: f = sqrt(f·f) для положительного f.

    Приём известный: квадрат удваивает относительную ошибку, корень ополовинивает
    её обратно — и заодно ополовинивает округление самого квадрата. Herbie находит
    его поиском; у нас он в e-граф не помещается, потому что правило на КАЖДОЕ
    положительное произведение раздувает граф до потолка и нужную комбинацию всё
    равно не рождает (проверено 26.09.2026, граф упёрся в 60 тысяч узлов).

    Поэтому делаем как с рядами Тейлора — отдельным кандидатом ПОСЛЕ извлечения.
    Граф не растёт вовсе, приём остаётся общим для любого положительного выражения,
    ...
    for pt in front:
        ...
        try:
            ...
            continue
        if not (iv[0] > 0.0):            # строго положительное значение на всём домене
            continue
        cand = ('sqrt', _square(tree))
        try:
            c2, e2, _, w2, l2 = tree_cost_refined(cand, domain, boxes=boxes)
        except (ValueError, ZeroDivisionError, OverflowError, KeyError):
            continue
        if math.isfinite(e2) and err is not None and e2 < err:
            out.append((c2, e2, cand) + tuple(pt[3:]))
    return out
...
def refine_front(front, domain, boxes=None):
    ...
    # Кандидаты «через квадрат» добавляются в ХВОСТ, порядок прежних точек цел.
    out = sqrt_square_candidates(out, domain, boxes=boxes)
    return out
